			...
			for (let i = 0; i < tdProperties.length; i++) {
				...
				for (let j = 0; j < curForms.length; j++) {
					...
					if (curForm.hasOwnProperty("security")) {
						...
						if (
							securityContains(securityDefinitions, curSecurity)
						) {
							// all good
						} else {
							results.push({
								...
								Status: "fail",
								Comment:
									"used a non defined security scheme in a property form",
							});
							return results;
						}
					}
					...
					if (curForm.hasOwnProperty("security")) {
						...
						if (
							...
							results.push({
								...
								Status: "fail",
								Comment:
									"used a non defined security scheme in an action form",
							});
							return results;
						}
					}
					...
					if (curForm.hasOwnProperty("security")) {
						...
						if (
							...
							results.push({
								...
								Status: "fail",
								Comment:
									"used a non defined security scheme in an event form",
							});
							return results;
						}
					}
...
function checkMultiLangConsistency(td) {
	...
	if (td.hasOwnProperty("titles")) {
		...
		isTdTitlesDescriptions.push({
			["root_title"]: isStringObjectKeyValue(td.title, rootTitlesObject),
		});
	}
	...
	if (td.hasOwnProperty("descriptions")) {
		...
		if (td.hasOwnProperty("description")) {
			//eslint-disable-next-line
			isTdTitlesDescriptions.push({
				["root_description"]: isStringObjectKeyValue(
					td.description,
					rootDescriptionsObject
				),
			});
		}
	}
		...
		for (let i = 0; i < tdProperties.length; i++) {
			...
			if (curProperty.hasOwnProperty("titles")) {
				...
				if (curProperty.hasOwnProperty("title")) {
					isTdTitlesDescriptions.push({
						["property_" + curPropertyName + "_title"]:
							isStringObjectKeyValue(
								curProperty.title,
								curProperty.titles
							),
					});
			...
			if (curProperty.hasOwnProperty("descriptions")) {
				...
				if (curProperty.hasOwnProperty("description")) {
					isTdTitlesDescriptions.push({
						["property_" + curPropertyName + "_desc"]:
							isStringObjectKeyValue(
	...
	for (let index = 0; index < isTdTitlesDescriptions.length; index++) {
		...
		if (element[elementName]) {
			...
			results.push({
				...
				Comment:
					elementName +
					" is not on the multilang object at the same level",
			});
			return results;
		}
	}
...
function checkUriSecurity(td) {
	...
	if (td.hasOwnProperty("securityDefinitions")) {
		...
		for (let index = 0; index < securityDefinitionsNames.length; index++) {
			const curSecurityDefinition =
				securityDefinitionsObject[securityDefinitionsNames[index]];
				...
				if (curSecurityDefinition.hasOwnProperty("in")) {
					if (curSecurityDefinition.in === "uri") {
						if (curSecurityDefinition.hasOwnProperty("name")) {
							securityUriVariables.push(
								curSecurityDefinition.name
							);
						}
					}
				}
			...
			if (td.hasOwnProperty("properties")) {
				...
				for (let i = 0; i < tdProperties.length; i++) {
					...
					if (curProperty.hasOwnProperty("uriVariables")) {
						let curPropertyUriVariables = Object.keys(
							curProperty.uriVariables
						);
						...
						if (curPropertyUriVariables.length > 0) {
							...
							const filteredArray =
								curPropertyUriVariables.filter((value) =>
			...
			if (td.hasOwnProperty("actions")) {
				...
				for (let i = 0; i < tdActions.length; i++) {
					...
					if (curAction.hasOwnProperty("uriVariables")) {
						let curActionUriVariables = Object.keys(
							curAction.uriVariables
						);
						curActionUriVariables.push(...rootUriVariables);
						if (curActionUriVariables.length > 0) {
							// there are urivariables somewhere at least
							// below is from https://stackoverflow.com/a/1885569/3806426
							const filteredArray = curActionUriVariables.filter(
								(value) => securityUriVariables.includes(value)
							);
			...
			if (td.hasOwnProperty("events")) {
				...
				for (let i = 0; i < tdEvents.length; i++) {
					...
					if (curEvent.hasOwnProperty("uriVariables")) {
						let curEventUriVariables = Object.keys(
							curEvent.uriVariables
						);
						curEventUriVariables.push(...rootUriVariables);
						if (curEventUriVariables.length > 0) {
							// there are urivariables somewhere at least
							// below is from https://stackoverflow.com/a/1885569/3806426
							const filteredArray = curEventUriVariables.filter(
								(value) => securityUriVariables.includes(value)
							);
