...
function tdValidator(
	tdString,
	logFunc,
	{ checkDefaults = true, checkJsonLd = true }
) {
	return new Promise((res, rej) => {
		...
		const detailComments = {
			enumConst:
				"Checking whether a data schema has enum and const at the same time.",
			propItems:
				"Checking whether a data schema has an object but not properties or array but no items.",
			security:
			...
			multiLangConsistency:
				"Checks whether all titles and descriptions have the same language fields.",
			linksRelTypeCount:
				"Checks whether rel:type is used more than once in the links array",
			readWriteOnly:
				"Warns if a property has readOnly or writeOnly set to true conflicting with another property.",
			uriVariableSecurity:
				"Checks if the name of an APIKey security scheme with in:uri show up in href and does not conflict with normal uriVariables",
		};
		...
		if (valid) {
			...
			if (checkDefaults) {
				...
				if (fullValid) {
					...
					logFunc(
						"> " + ajv.errorsText(filterErrorMessages(ajv.errors))
					);
					res({ report, details, detailComments, validationErrors });
				}
			}
			...
			details.propUniqueness = evalAssertion(
				checkPropUniqueness(tdString)
			);
			if (details.propUniqueness === "passed") {
				details.propUniqueness = checkSecPropUniqueness(
					tdString,
					tdJson
				);
			} else {
			...
			details.multiLangConsistency = evalAssertion(
				checkMultiLangConsistency(tdJson)
			);
			details.linksRelTypeCount = evalAssertion(
				checkLinksRelTypeCount(tdJson)
			);
			details.uriVariableSecurity = evalAssertion(
				checkUriSecurity(tdJson)
			);
			...
			validationErrors.schema = ajv.errorsText(
				filterErrorMessages(ajv.errors)
			);
			...
			jsonld
				...
				.then(
					...
					(err) => {
						...
						logFunc(
							"Hint: Make sure you have internet connection available."
						);
						logFunc("> " + err);
						res({ report, details, detailComments });
					}
		...
		function checkEnumConst(td) {
			...
			if (td.hasOwnProperty("properties")) {
				...
				for (let i = 0; i < tdProperties.length; i++) {
					...
					if (
						curProperty.hasOwnProperty("enum") &&
						curProperty.hasOwnProperty("const")
					) {
			...
			if (td.hasOwnProperty("actions")) {
				...
				for (let i = 0; i < tdActions.length; i++) {
					...
					if (curAction.hasOwnProperty("input")) {
						...
						if (
							curInput.hasOwnProperty("enum") &&
							curInput.hasOwnProperty("const")
						) {
							details.enumConst = "warning";
							logFunc(
					...
					if (curAction.hasOwnProperty("output")) {
						...
						if (
							curOutput.hasOwnProperty("enum") &&
							curOutput.hasOwnProperty("const")
						) {
							details.enumConst = "warning";
							logFunc(
			...
			if (td.hasOwnProperty("events")) {
				...
				for (let i = 0; i < tdEvents.length; i++) {
					...
					if (
						curEvent.hasOwnProperty("enum") &&
						curEvent.hasOwnProperty("const")
					) {
						details.enumConst = "warning";
						logFunc(
							"! Warning: In event " +
			...
			if (td.hasOwnProperty("properties")) {
				...
				for (let i = 0; i < tdProperties.length; i++) {
					...
					if (curProperty.hasOwnProperty("type")) {
						if (
							curProperty.type === "object" &&
							!curProperty.hasOwnProperty("properties")
						) {
						...
						if (
							curProperty.type === "array" &&
							!curProperty.hasOwnProperty("items")
						) {
							details.propItems = "warning";
							logFunc(
								"! Warning: In property " +
				...
				for (let i = 0; i < tdActions.length; i++) {
					...
					if (curAction.hasOwnProperty("input")) {
						...
						if (curInput.hasOwnProperty("type")) {
							if (
								curInput.type === "object" &&
								!curInput.hasOwnProperty("properties")
							) {
								details.propItems = "warning";
								logFunc(
									"! Warning: In the input of action " +
					...
					if (curAction.hasOwnProperty("output")) {
						...
						if (curOutput.hasOwnProperty("type")) {
							if (
								curOutput.type === "object" &&
								!curOutput.hasOwnProperty("properties")
							) {
								details.propItems = "warning";
								logFunc(
									"! Warning: In the output of action " +
							...
							if (
								curOutput.type === "array" &&
								!curOutput.hasOwnProperty("items")
							) {
								details.propItems = "warning";
								logFunc(
									"! Warning: In the output of action " +
			...
			if (td.hasOwnProperty("events")) {
				...
				for (let i = 0; i < tdEvents.length; i++) {
					...
					if (curEvent.hasOwnProperty("type")) {
						if (
							curEvent.type === "object" &&
							!curEvent.hasOwnProperty("properties")
						) {
							details.propItems = "warning";
							logFunc(
								"! Warning: In event " +
		...
		function checkReadWriteOnly(td) {
			...
			if (td.hasOwnProperty("properties")) {
				...
				for (let i = 0; i < tdProperties.length; i++) {
					...
					// if readOnly is set
					if (
						curProperty.hasOwnProperty("readOnly") &&
						curProperty.readOnly === true
					) {
						// check if both readOnly and writeOnly are true
						if (
							curProperty.hasOwnProperty("writeOnly") &&
							curProperty.writeOnly === true
						) {
						...
						if (curProperty.hasOwnProperty("forms")) {
							for (const formElIndex in curProperty.forms) {
								if (
									curProperty.forms.hasOwnProperty(
										formElIndex
									)
								) {
									const formEl =
										curProperty.forms[formElIndex];
									if (formEl.hasOwnProperty("op")) {
										if (
											(typeof formEl.op === "string" &&
												formEl.op ===
													"writeproperty") ||
					...
					if (
						...
						if (
							curProperty.hasOwnProperty("observable") &&
							curProperty.observable === true
						) {
							details.readWriteOnly = "warning";
							logFunc(
								"! Warning: In property " +
		...
		function checkSecPropUniqueness(tdStr, td) {
			...
			try {
				...
				if (td.securityDefinitions.hasOwnProperty(securitySchemeName)) {
					result = "failed";
					logFunc(
						"KO Error: The securityDefinitions contain a duplicate"
					);
				}
			}
		...
		function filterErrorMessages(errors) {
			...
			errors.forEach((el) => {
				if (
					!output.some(
						(ce) =>
							ce.dataPath === el.dataPath &&
							ce.message === el.message
					)
				) {
					output.push(el);
				}
	...
	return new Promise((res, rej) => {
		...
		const detailComments = {
			...
			readWriteOnly:
				"Warns if a property has readOnly or writeOnly set to true conflicting with another property.",
			tmOptionalPointer:
				"Checking whether tm:optional points to an actual affordance",
		};
		...
		if (valid) {
			...
			details.multiLangConsistency = evalAssertion(
				checkMultiLangConsistency(tmJson)
			);
			details.linksRelTypeCount = evalAssertion(
				checkLinksRelTypeCount(tmJson)
			);
			details.tmOptionalPointer = evalAssertion(
				checkTmOptionalPointer(tmJson)
			);
		...
		for (const key in properties) {
			if (properties.hasOwnProperty(key)) {
				if (key === REF) {
					if (
						path[0] === "r" &&
						properties[key].includes(DATA_SCHEMA)
					) {
						continue;
					}
					...
					findPathsInSchema(
						getRefObjectOfSchema(tdSchema, properties[key]),
						path
					);
					return;
				} else {
		...
		for (const key in additionalProperties) {
			if (additionalProperties.hasOwnProperty(key)) {
				if (key === REF) {
					if (
						path[0] === "r" &&
						additionalProperties[key].includes(DATA_SCHEMA)
					) {
						continue;
					}
		...
		for (const item in items) {
			if (items.hasOwnProperty(item)) {
				if (item === REF) {
					...
					findPathsInSchema(
						getRefObjectOfSchema(tdSchema, items[item]),
						path
					);
					return;
				}
...
function calculateSimilarity(actual, desired) {
	...
	for (let i = 0; i < actual.length; i++) {
		...
		const upperBound =
			i + range <= desired.length ? i + range : desired.length - 1;

		for (let j = lowerBound; j <= upperBound; j++) {
			if (
				actualMatches[i] !== true &&
				desiredMatches[j] !== true &&
				actual[i] === desired[j]
			) {
				m++;
